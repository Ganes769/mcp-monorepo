...
import { IconHome, IconIssues, IconProjects, IconSettings, IconStandup } from './icons'
...
export function Sidebar({ active, onChange, projectKey, projectName, healthy }: Props) {
  const { classes } = theme
        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={cx(
                'flex items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left text-[13px] font-semibold transition',
                selected
        ...
        <div className="flex items-center gap-2">
          <span
            className={cx(
              ...
              healthy ? 'bg-emerald-500' : healthy === false ? 'bg-rose-500' : 'bg-slate-300',
            )}
          />
          <span className={classes.muted}>
            {healthy ? 'API connected' : healthy === false ? 'API offline' : 'Connecting'}
          </span>
        </div>

...
const highlights = [
  { step: '01', title: 'Daily standup', body: 'Issues grouped by blocked, in progress, to do, and done.' },
  { step: '02', title: 'Slack delivery', body: 'The same summary posts to your channel on a schedule.' },
  { step: '03', title: 'Jira source of truth', body: 'Nothing is copied out of your existing project.' },
]
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-950">
      <header className="absolute inset-x-0 top-0 z-20 px-5 pt-5 sm:px-8">
        <div className="mx-auto flex max-w-[1120px] items-center justify-between rounded-2xl border border-white/15 bg-white/10 px-4 py-3 backdrop-blur-md">
          <p className="font-display text-lg font-semibold tracking-tight text-white">WorkBridge</p>
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={onOpenApp}
              className="hidden rounded-lg px-3.5 py-2 text-sm font-semibold text-white/90 transition hover:bg-white/10 sm:inline-flex"
            >
              Open app
            </button>
            <button
              ...
              className="rounded-lg bg-white px-3.5 py-2 text-sm font-semibold text-slate-950"
            >
              Connect your org
            </button>
          </div>
      ...
      <section className="relative h-[94vh] min-h-[680px] overflow-hidden">
        <div
          className="hero-kenburns absolute inset-0 bg-cover bg-center"
          style={{ backgroundImage: `url(${peopleWorking})` }}
        />
        <div className="absolute inset-0 bg-[#06122e]/70" />
        <div className="absolute inset-0 bg-gradient-to-r from-[#06122e] via-[#06122e]/80 to-transparent" />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_20%_20%,rgba(56,189,248,0.16),transparent_42%)]" />

        <div className="relative z-10 mx-auto flex h-full max-w-[1120px] items-center px-5 pb-20 pt-24 sm:px-8">
          <div className="max-w-[620px]">
            ...
            <p className="hero-fade-up-delay-1 mb-8 max-w-[520px] text-lg leading-7 text-slate-200">
              Connect Jira, group the board, and send a clear morning brief to Slack.
              No extra project tool. No copied tickets.
            </p>
            <div className="hero-fade-up-delay-2 flex flex-col gap-3 sm:flex-row">
              <button
                type="button"
                onClick={onConnectOrg}
                className="rounded-lg bg-blue-600 px-6 py-3 text-[15px] font-semibold text-white shadow-[0_12px_30px_rgba(37,99,235,0.35)] transition hover:-translate-y-0.5 hover:bg-blue-500"
              >
                Connect your org
              </button>
              <button
                type="button"
                onClick={openDemo}
                className="rounded-lg border border-white/25 bg-white/5 px-6 py-3 text-[15px] font-semibold text-white backdrop-blur-sm transition hover:bg-white hover:text-slate-950"
              >
                Book a demo
              </button>
            </div>
        ...
        <div className="scroll-cue absolute bottom-7 left-1/2 z-10 flex -translate-x-1/2 flex-col items-center gap-1.5 text-white/60">
          <span className="text-[11px] font-semibold uppercase tracking-[0.22em]">Scroll</span>
      ...
      <section className="relative z-10 mx-auto -mt-10 max-w-[1120px] px-5 sm:px-8">
        <div className="grid grid-cols-2 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_20px_50px_rgba(15,23,42,0.08)] sm:grid-cols-4">
          {stats.map((item, index) => (
            ...
            >
              <p className="font-display text-xl font-semibold tracking-tight text-slate-950">{item.value}</p>
        ...
        <div className="mx-auto flex max-w-[1120px] flex-col items-center gap-16 lg:flex-row">
          ...
          <div className="flex-1">
            ...
            <p className="mb-8 text-[17px] leading-7 text-slate-600">
              WorkBridge reads the live Jira board, writes a professional standup, and
              delivers it to Slack. Your team stays in the tools they already use.
            </p>
            <ol className="space-y-4">
              {['Connect Jira and Slack', 'Choose the project to brief', 'Post or schedule the standup'].map(
                (item, index) => (
      ...
      <section className="px-5 pb-24 sm:px-8">
        <div className="mx-auto grid max-w-[1120px] grid-cols-1 gap-4 md:grid-cols-3">
          {highlights.map((item) => (
            ...
            >
              <p className="mb-6 text-[12px] font-semibold tracking-[0.14em] text-blue-700">{item.step}</p>
        ...
        <div className="mx-auto grid max-w-[1120px] overflow-hidden rounded-3xl bg-[#06122e] lg:grid-cols-2">
          <div className="px-8 py-12 sm:px-12">
            ...
            <p className="max-w-md text-[15px] leading-7 text-slate-300">
              A short walkthrough of connect, briefing, and Slack delivery. No setup
      ...
      <SiteFooter onConnectOrg={onConnectOrg} onOpenApp={onOpenApp} onBookDemo={openDemo} />
    </div>
  )
}
